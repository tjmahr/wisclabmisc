...
`pROC::roc()` does not support observation weights, so this package
provides `compute_sens_spec_from_ecdf()` to compute sensitivity and
specificity using (weighted) ECDFs. If not weights are provided, the
normal unweighted results are used. This function does not use
`pROC::roc()` at all.

To simulate weighting, we will round the existing predictor and count the 
number of observations in each bin (rounded value). That count is the weight.
